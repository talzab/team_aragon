    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                if check_duplicate_id(curr, 'Hospitals', 'hospital_pk', row['hospital_pk']):
                    raise ValueError(f"Duplicate ID found in Hospitals table: {row['hospital_pk']}")
            ...
            try:
                ...
                if check_duplicate_id(curr, 'HospitalLocations', 'hospital_fk', row['hospital_pk']):
                    raise ValueError(f"Duplicate ID found in HospitalLocation table: {row['hospital_pk']}")
            ...
            try:
                ...
                if check_duplicate_id(curr, 'HospitalBedInformation', 'hospital_fk', row['hospital_pk'], 'collection_week', row['collection_week']):
                    raise ValueError(f"Duplicate ID and Collection Date found in HospitalBedInformation table: {row['hospital_pk'], row['collection_date']}")
